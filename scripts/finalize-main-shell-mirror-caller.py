#!/usr/bin/env python3
"""Bind the protected main-shell mirror caller to one exact authority tuple.

The checked-in caller template is intentionally inert. This helper previews
the complete Mpre/TF/C replacement by default and writes only with --apply.
"""
...
TAP_PLACEHOLDER = "__FINAL_TAP_CATALOG_SHA__"
CANARY_PLACEHOLDER = "__FINAL_CANARY_SHA__"
...
def build_finalization(
    ...
    *,
    ...
    requested = {
        "Kandelo Mpre": kandelo_sha,
        "tap catalog TF": tap_catalog_sha,
        "canary C": canary_sha,
    }
    ...
    for pattern, placeholder, replacement, label in (
        ...
        (CALLER_TAP, TAP_PLACEHOLDER, tap_catalog_sha, "caller input TF"),
        (CALLER_CANARY, CANARY_PLACEHOLDER, canary_sha, "caller input C"),
    ):
    ...
    for pattern, placeholder, replacement, label in (
        ...
        (TRUST_TAP, TAP_PLACEHOLDER, tap_catalog_sha, "trust TF"),
        (TRUST_CANARY, CANARY_PLACEHOLDER, canary_sha, "trust C"),
    ):
    ...
    for placeholder in (
        ...
        if placeholder in caller:
            raise FinalizationError(
                f"{CALLER_PATH} still contains unresolved placeholder {placeholder}"
            )
    ...
    if (
        ...
        raise FinalizationError("final caller does not contain exactly one TF and C")
    forbidden = (
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--tap-catalog-sha", required=True)
...
def main(argv: list[str]) -> int:
    ...
    try:
        finalization = build_finalization(
            ...
            tap_catalog_sha=args.tap_catalog_sha,
            canary_sha=args.canary_sha,
        )
        if args.apply:
        ...
        if finalization.changed:
            ...
            print(f"{action}: caller already matches the requested Mpre/TF/C tuple")
        return 0
