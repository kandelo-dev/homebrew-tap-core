...
TF = "b123456789012345678901234567890123456789"
C = "c123456789012345678901234567890123456789"
...
class MainShellMirrorCallerFinalizationTests(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.force_scalar(
            finalizer.CALLER_PATH,
            finalizer.CALLER_TAP,
            finalizer.TAP_PLACEHOLDER,
        )
        self.force_scalar(
            finalizer.CALLER_PATH,
            finalizer.CALLER_CANARY,
            finalizer.CANARY_PLACEHOLDER,
        )
        for pattern, placeholder in (
            (finalizer.TRUST_KANDELO, finalizer.MPRE_PLACEHOLDER),
            (finalizer.TRUST_TAP, finalizer.TAP_PLACEHOLDER),
            (finalizer.TRUST_CANARY, finalizer.CANARY_PLACEHOLDER),
        ):
            self.force_scalar(finalizer.TRUST_PATH, pattern, placeholder)
    ...
    def test_trust_first_partial_application_keeps_caller_inert(self) -> None:
        ...
        self.assertIn(finalizer.TAP_PLACEHOLDER, caller)
        self.assertIn(finalizer.CANARY_PLACEHOLDER, caller)
    ...
    def test_malformed_requested_refs_are_rejected(self) -> None:
        for keyword in ("kandelo_sha", "tap_catalog_sha", "canary_sha"):
            arguments = {
                "kandelo_sha": MPRE,
                "tap_catalog_sha": TF,
                "canary_sha": C,
            }
            arguments[keyword] = "A" * 40
            with self.subTest(keyword=keyword), self.assertRaisesRegex(
                finalizer.FinalizationError,
                "40 lowercase hex",
            ):
                finalizer.build_finalization(self.root, **arguments)
