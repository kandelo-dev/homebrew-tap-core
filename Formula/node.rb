...
class Node < Formula
  ...
  # WHY: argv[0] selects Node mode inside the shared engine. Derive every
  # installed command identity from Kandelo's one guest-prefix authority so a
  # native Homebrew-on-Linux host path cannot leak into the bottle contract.
  GUEST_OPT_PREFIX =
    "#{KandeloFormulaSupport::KANDELO_GUEST_HOMEBREW_PREFIX}/opt/node".freeze
