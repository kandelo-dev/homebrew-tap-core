...
class Npm < Formula
  ...
  # WHY: These strings become launcher shebangs and runtime module paths.
  # Derive both from Kandelo's one guest-prefix authority so the bottle never
  # captures the trusted build host's native Homebrew-on-Linux prefix.
  GUEST_OPT_PREFIX =
    "#{KandeloFormulaSupport::KANDELO_GUEST_HOMEBREW_PREFIX}/opt/npm".freeze
  GUEST_NODE =
    "#{KandeloFormulaSupport::KANDELO_GUEST_HOMEBREW_PREFIX}/opt/node/bin/node".freeze
