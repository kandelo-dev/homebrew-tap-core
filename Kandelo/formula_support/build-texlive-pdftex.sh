...
verify_object_archive() {
  ...
  if ! listing="$(wasm32posix-ar t "${archive}")"
  then
...
build_engine() {
  ...
  if [[ ! -f "${libpng_prefix}/lib/libpng16.a" && ! -f "${libpng_prefix}/lib/libpng.a" ]]
  then
  ...
  local sdk_config_site_mprotect_contract="ac_cv_func_mprotect=\${ac_cv_func_mprotect=no}"
  require_file "${sdk_config_site}"
  if ! grep -Fq "${sdk_config_site_mprotect_contract}" "${sdk_config_site}"
  then
    echo "ERROR: Kandelo SDK config.site does not preserve mprotect=no" >&2
    exit 1
  fi
  export CONFIG_SITE="${sdk_config_site}"
  if [[ "${CONFIG_SITE}" != "${kandelo_root}/sdk/config.site" ]]
  then
  ...
  if [[ "${zlib_pkg_flags}" != *"${zlib_realpath}"* || "${libpng_pkg_flags}" != *"${libpng_realpath}"* ]]
  then
  ...
  if [[ "${kandelo_realpath}" != "${kandelo_root}" ]]
  then
  ...
  if [[ "${libpng_realpath}" != "${libpng_prefix}" ]]
  then
  ...
  if [[ "${libcxx_realpath}" != "${libcxx_prefix}" ]]
  then
  ...
  if [[ ! -x "${cross_build_dir}/texk/web2c/CXXLD.sh" ]] ||
     ! grep -Fq "${kpse_cv_cxx_flags}" "${cross_build_dir}/texk/web2c/CXXLD.sh"
  then
...
build_formats() {
  ...
  local pdf_header
  pdf_header="$(head -c 5 "${output_dir}/input.pdf")"
  if [[ "${pdf_header}" != "%PDF-" ]]
  then
    echo "ERROR: host pdfTeX smoke output is not a PDF" >&2
    exit 1
  fi
  if ! tail -c 1024 "${output_dir}/input.pdf" | grep -a -q '%%EOF'
  then
    echo "ERROR: host pdfTeX smoke output has no PDF EOF marker" >&2
    exit 1
  fi
  local dvi_preamble
  dvi_preamble="$(od -An -tu1 -N2 "${output_dir}/latex-input.dvi")"
  if [[ ! "${dvi_preamble}" =~ ^[[:space:]]*247[[:space:]]+2[[:space:]]*$ ]]
  then
