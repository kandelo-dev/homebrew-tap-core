...
[ "${WASM_POSIX_DEP_TARGET_ARCH:?}" = "wasm32" ] || {
    echo "ERROR: NetHack is currently packaged for wasm32 only" >&2
    exit 2
}
[ "$NETHACK_VERSION" = "3.6.7" ] &&
    ...
    echo "ERROR: NetHack source identity differs from the reviewed recipe" >&2
    exit 2
}
...
# --- Use the exact Homebrew-poured ncurses keg selected by the publisher. ---
...
echo "  staged runtime tree into Formula output"
