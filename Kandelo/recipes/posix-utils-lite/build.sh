...
if [ "${WASM_POSIX_DEP_NAME:?}" != "posix-utils-lite" ] ||
   ...
   [ "${WASM_POSIX_DEP_TARGET_ARCH:?}" != "wasm32" ]; then
    echo "ERROR: posix-utils-lite Formula identity differs from the reviewed tap recipe" >&2
    exit 2
fi

SOURCE="$RECIPE_DIR/src/posix-utils-lite.c"
if [ ! -f "$SOURCE" ] || [ -L "$SOURCE" ]; then
    echo "ERROR: posix-utils-lite source is not a direct regular file: $SOURCE" >&2
    exit 2
fi
utilities=(
  ...
  pgrep ps renice strings strip uncompress uudecode uuencode what xgettext yacc
)
mkdir -p "$WORK_DIR" "$OUT_DIR"
wasm32posix-cc -std=c11 -D_POSIX_C_SOURCE=200809L -O2 -Wall -Wextra \
    -Wno-unused-parameter "$SOURCE" -o "$WORK_DIR/posix-utils-lite.wasm"
for utility in "${utilities[@]}"; do
    install -m 0755 "$WORK_DIR/posix-utils-lite.wasm" "$OUT_DIR/$utility.wasm"
done
